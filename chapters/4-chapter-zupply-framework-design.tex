...
%	The Merkle Hash Tree provides collision resistance, meaning that it is computationally infeasible for any \gls{ppt} adversary to find two different sets of leaves that produce the same Merkle root. 
%\end{lemma}
%\begin{proof}
...
%	 A valid Merkle proof ($\text{Path}_i$) and Merkle root ($\texttt{rt}$) for a leaf with index $i$ uniquely determine the leaf at index $i$. In other words, it is computationally infeasible for any \gls{ppt} adversary to generate the same valid Merkle proof and root combination for two different leaves in a Merkle tree.
%\end{lemma}
%\begin{proof}
%    Due to the cryptographic properties of hash functions and the construction of Merkle trees, it is computationally impractical for any \gls{ppt} adversary to generate the same valid Merkle proof and root combination for two different leaves.
%\end{proof}
...
To formalize more these privacy properties, we assume three properties for the commitments to \glspl{aat}: \textbf{attribution}, \textbf{ownership} and \textbf{class}. A commitment \texttt{cm} is attributed to an entity $e$ if and only if $e$ uploads \texttt{cm}; this is denoted as
\[
...
Moreover, we assume a \textbf{commitment} property for each data record $d$, denoted as 
\[
d.\texttt{cm}=\texttt{cm}, 
\]
which links the data record to its corresponding commitment. This implies that the data record $d$ includes a \gls{zkp} demonstrating knowledge of the value committed to in \texttt{cm}. Also, $d$ is signed using the secret key associated with the \gls{aat} that is committed to in \texttt{cm}.
	...
	\centering
	    \begin{subfigure}[t]{0.35\textwidth}
	    	\centering
	    		...
	    		\node (e1) {${e_1}$ initiates an \gls{aat}};
	    		
	    			   			\node[draw, rounded corners,below=1cm of $(e1)$] (txInit) {
	    			\begin{tabular}{c}
	    				Transaction $\texttt{tx}_\mathsf{Init}$ \\
	    				\hline
	    				$\texttt{cm\_new} = \texttt{cm}_1$ \\
	    				$\texttt{attr} = e_1$ 
	    			\end{tabular}
	    		};
	    		
	    		\node[draw, rounded corners,below=1.4cm of $(txInit)$] (cm) {
	    			\begin{tabular}{c}
	    				Commitment $\texttt{cm}_1$ \\
	    				\hline
	    				$\texttt{attr} = e_1$ \\
	    				$\texttt{owner} = e_1$ \\
	    				$\texttt{class} = \mathsf{Init}$
	    			\end{tabular}
	    		};
	    		
	    			   			\node[below=2cm of $(cm)$,xshift=.5cm](note){\	 };
	    		
%	    		\draw[->, thick, >=Stealth] (e1) -- node[above,sloped] {$\texttt{tx}_\textsf{Init}$} (cm);

	    	\end{tikzpicture}

	    	\caption{$\texttt{cm}_1$ is created through $\texttt{tx}_\textsf{Init}$}
	    		    		\label{fig:attr-owner-cm-txinit}
	   	\end{subfigure}%
	   		...
	   		\begin{tikzpicture}[node distance=3.5cm]
	   			...
	   			\node[draw, rounded corners,below=1cm of $(e1)!0.5!(e2)$] (txTrans) {
	   				\begin{tabular}{c}
	   					Transaction $\texttt{tx}_\mathsf{Trans}$ \\
	   					\hline
	   					{\scriptsize\faLock[regular]{}} $\texttt{cm\_old} = \texttt{cm}_1$ \\
	   					$\texttt{cm\_new} = \texttt{cm}_2$ \\
	   					$\texttt{attr} = e_1$ 
	   				\end{tabular}
	   			};
	   			
	   			\node[draw, rounded corners,below=1.4cm of $(txTrans)$,xshift=2.2cm] (cm2) {
	   				\begin{tabular}{c}
	   					Commitment $\texttt{cm}_2$ \\
	   					\hline
	   					$\texttt{attr} = e_1$ \\
	   					{\scriptsize\faLock[regular]{}} $\texttt{owner} = e_2$ \\
	   					$\texttt{class} = \mathsf{Trans}$
	   				\end{tabular}
	   			};
	   			
	   			\node[draw, rounded corners,below=1.4cm of $(txTrans)$,xshift=-2.2cm] (cm1) {
	   				\begin{tabular}{c}
	   					Commitment $\texttt{cm}_1$ \\
	   					\hline
	   					$\texttt{attr} = \ast$\\
	   					{\scriptsize\faLock[regular]{}} $\texttt{owner} = e_1$ \\
	   					$\texttt{class} = \ast$
	   				\end{tabular}
	   			};
	   			\node[below=1.2cm of $(cm1)$,xshift=.5cm](note){
	   				\begin{tabular}{l}
	   					{\scriptsize\faLock[regular]{}} \footnotesize{denotes a confidential value}\\
	   					 \footnotesize{$\ast$ denotes a context-dependent value}
	   				\end{tabular}};
	   			
%	   			\draw[->, thick, >=Stealth] (e1) -- node[above,sloped] {$\texttt{tx}_\textsf{Trans}$} (cm);
	   			\draw[->, thick, >=Stealth, bend left=10] (e1) to node[above] {ownership transfer} (e2);
...
%we define the core privacy-preserving properties required by the framework in Section~\ref{sec:Required Security Properties}, following the presentation of the threat model.

%This section also presents an indistinguishability experiment, where the adversary $\mathcal{A}$  builds two ledgers which are publicly consistent (i.e., the public values are the same) through a challenger. 

%The entities used as the owners of  the \gls{aat} commitments published through  transactions of type \textsf{Trans}, \textsf{Merge}, or \textsf{Div} are not public and the random looking data (including the Merkle roots on data records) are either sampled or simulated. 
%Hence, the $\mathcal{A}$  should not be able to distinguish these two ledgers. Consequently, the adversary cannot determine that a data record corresponds to which ledger and is not able to determine the preceding commitment and the owner of  a $\texttt{cm}_j$ that is published via a transaction of type \textsf{Trans}, \textsf{Merge}, or \textsf{Div}.
%\begin{itemize}
%	\item   Note
%\end{itemize}


%  This property is going to be expressed using the indistinguishability experiment presented in Section~\ref{sec:ind_experiment}  
...
%The superior adversary $\mathcal{A}_s$ inherits all the capabilities of $\mathcal{A}_c$, and additionally has access to the \gls{aat} commitment attribution oracle $\mathcal{O}^\mathsf{Attr}$, which will be detailed later in this section. Below, we outline the assumptions associated with the common adversary.
...
%We formally define this assumption later in Section \ref{sec:ind_experiment}. 
...
%	(i.e., $\texttt{tx}.\texttt{cm\_new} = \texttt{cm}$ and $\texttt{cm}.\texttt{class} \in \{\textsf{Trans}, \textsf{Merge}, \textsf{Div}\}$)
...
\subsection{Required Security Properties} \label{sec:Required Security Properties}
...
%	\item \textit{Strong data anonymity}: The Zupply framework has strong data anonymity, if $\mathcal{A}_S$ has a negligible chance to success the data de-anonymization attack.
...
%	\item \textit{Strong token anonymity}: The Zupply framework has strong token anonymity, if $\mathcal{A}_E$ has a negligible chance to success the token de-anonymization attack.
...
Now, we consider different scenarios where \texttt{p-set} and \texttt{t-set} are not both empty:
\begin{enumerate}
...
In all other cases, \( \texttt{cm}^*.\texttt{attr} \) may correspond to any entity in \( \mathbf{E}_\tau \), implying that \( \texttt{cm}^* \) could be any commitment in \( \mathbf{C}_\tau \) whose \texttt{class} is not \( \mathsf{Init} \). Thus, \( \mathcal{A}_S \) can discard those commitments in \( \mathbf{C}_\tau \) whose \texttt{class} is \( \mathsf{Init} \) and whose \texttt{attr} is not \( e \), since \( e \) is not their owner and therefore cannot transfer their ownership. However, \( \mathcal{A}_S \) does not have any further advantage in determining the ownership of the remaining commitments. Hence, the probability of correctly identifying \( \texttt{cm}^* \) among the remaining commitments in \( \mathbf{C}_\tau \) remains negligible.
...
%Zupply adopted a stronger adversary model that previous methods, in which the underlying blockchain platform does not preserve the anonymity of the transaction issuer (adversary's access to $\mathcal{O}^\mathsf{Attr}$). 
