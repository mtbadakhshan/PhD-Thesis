...
All algorithms have read-only access to $\mathbf{TX}_\tau$, $\mathbf{C}_\tau$, and $\mathbf{X}_\tau$, and $\mathbf{D}_\tau$ (via \textsf{DCS} queries). The smart contract and \textsf{DCS} can update these sets when the \textsf{VerifyTX} or \textsf{Audit} algorithms output~1.
...
%\subsection{Indistinguishability Experiment}
In this section, we do not provide security proofs, deferring them to future work. Instead, we present the experimental setup for evaluating ledger indistinguishability. To characterize the ledger indistinguishability of the Zupply framework, we design an \textsf{L-IND} experiment following the similarly named experiment in Zerocash~\cite{zcash2014}.  
Recall that a ledger $L$ includes a set of transactions $\mathbf{TX}$. From $\mathbf{TX}$, we extract the ordered set $\mathbf{C}$, which stores commitments to \gls{aat}s (\texttt{cm}s), and the set of \gls{eol}s, denoted as $\mathbf{X}$. Each \texttt{cm} in $\mathbf{C}$  can be replaced by a tuple $(\texttt{cm}, \text{TXtype}, e)$, where $\texttt{cm}$ is the commitment added to the ledger by an entity $e$ through a transaction of type TXtype. The transaction type can be either \textsf{Trans} or \textsf{Init}. We call this new $\mathbf{C}$ as its generalized form.  Notably, if $\text{TXtype} = \textsf{Trans}$, the entity $e$ does not \textit{own} the corresponding \gls{aat}; rather, it only adds it to the ledger. The adversary’s capabilities determine whether they can extract the entity $e$ associated with each commitment in the transactions. For simplicity, we first assume that the adversary lacks this capability.
...
\begin{enumerate}
	\item The set $\mathbf{C}$ in its generalized form.
...
The oracle $\mathcal{O}^\Pi$ is initialized by public parameters \textsc{pp} and can receive and answer the following queries:
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item  Add $(\texttt{cm}, \textsf{Init}, e)$ to $\mathbf{C}$.
...
%% TRANS
	...
	\begin{enumerate}
		...
		\item \sloppy Compute \textsf{Trans}$\big($\textsc{pp}$, T^{e^\text{old}},  \text{PKsig}^\text{new} \big)$ $\rightarrow$ \big($\texttt{tx}_{\textsf{Trans}}, \Tilde{T}^{e^\text{new}}, \texttt{Tag}\big)$, where $\texttt{tx}_{\textsf{Trans}} = \big(\pi_{\textsf{Trans}}, x_{\textsf{Trans}}, \texttt{eol}, \texttt{cm}, \texttt{rt}^{\text{new}}, \mathsf{ind}, \mathsf{path}_{\mathsf{ind}} \big)$
		\item  Add $(\texttt{cm}, \textsf{Trans}, e^\text{old})$ to $\mathbf{C}$.
		\item Add $T^{e^\text{new}} = (\Tilde{T}^{e^\text{new}},  \text{PKsig}^\text{new})$ to $\mathbf{T}^{e^{\text{new}}}$
	\end{enumerate}
	
	\item $Q(\textbf{Upload}, \text{IsTransferred}, d^\text{old}, d_{\text{pub}, \text{pri}}^\text{new},  e^\text{new})$, where $d_{\text{pub}, \text{pri}}^\text{new}$ represents the public and private part of the new data to be added to $D$, succeeding $d^\text{old}$, which is already in $D$. The Boolean variable $\text{IsTransferred}$ indicates whether the new data has a different $\text{PKsig}$ than $d^\text{old}$; it is set to \texttt{True} if $\text{PKsig}$ differs and \texttt{False} otherwise.\footnote{The query $Q(\textbf{Upload}, \text{False}, \text{Null}, d_{\text{pub}, \text{pri}}^\text{new},  e^\text{new})$ initiates a \gls{dag}.}
	
	\begin{enumerate}
		\item Assert $d^\text{old}$ is either Null or is in $D$, otherwise output $\perp$.
		...
		\item If $\text{IsTransferred} = \texttt{True}$, then commit to $T^\text{old}$ to compute $\texttt{cm}^\text{old}$, and run  $Q(\textbf{Trans}, \texttt{cm}^\text{old}, e^\text{new})$ to produce $T^{e^\text{new}}$. Otherwise, $T^{e^\text{new}} = T^\text{old}$.
	...
	\item $Q(\textbf{Insert}, \texttt{tx})$, where $\texttt{tx}$ is either the type of \textsf{Trans} or  \textsf{Init}.
	\begin{enumerate}
...
Now, in the \textsf{L-IND} experiment, the challenger $\mathcal{C}$ instantiate two oracles denoted as  $\mathcal{O}_0^\Pi$ and $\mathcal{O}_1^\Pi$. Then, $\mathcal{C}$ randomly picks $b \in \{0,1\}$ and provides two ledgers $L_\text{Left} = L_b$  and  $L_\text{Right} = L_{1-b}$ to $\mathcal{A}$. A sends two queries of $Q$ and $Q'$ to the challenger where they are publicly consistent, meaning that every public values in those two queries are the same.	 
The challenger sends $Q$ to $L_0$ and $Q'$ to $L_1$. The adversary's advantage in this experiment is the probability that the adversary can guess $b$.
